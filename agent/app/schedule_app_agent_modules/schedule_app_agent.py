...
from agent.app.schedule_app_agent_modules.agent_import import agent_import


class ScheduleAppAgent(AgentNode):
    ...
    def get_prompt(self, chat_no=None):
        content = f"""
            ...
            已知Agent信息:
                - ScheduleCronAgent -> 处理cron格式定时任务agent
        ...
        if json_data["code"] != 0:
            ...
            if agent_name:
                ...
                agent_instance.appendMessage(chat_no,
                                             {"role": "user", "content": json.dumps(self.messageDict[chat_no][0:-1])})
