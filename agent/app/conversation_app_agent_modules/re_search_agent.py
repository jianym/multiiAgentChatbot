...
from agent.app.conversation_app_agent_modules.search_agent import instance as search
...
class ReSearchAgent(AgentNode):
    ...
    def get_prompt(self, chat_no=None):
        content = f"""
        你是一名ReAct助手，你的目标严格遵守执行流程回答用户问题。
            
        基本信息:
            {self.get_context_data(chat_no,"baseInfo")}
            
        可调用工具:
          - `query_search(question: str)`: 对问题进行检索，返回问题的检索结果
            - `question`: 字符串类型， 需要检索的问题
            
        你必须严格按照如下步骤流程执行， 不可跳过中间流程，如果需要回退执行，则必须回退到相关步骤执行，每个步骤都要返回执行后结果，当前步骤结果返回后执行下一步骤操作:
            1. infer: 根据上下文信息提出要检索的问题，并调用相关工具传递相关参数
            2. Observation：从多维度，多角度分析当前检索结果的优缺点及内容是否详细，来源是否可靠，覆盖是否全面
                 1.如果不能很好的解决用户问题，针对缺点，回退到步骤1.infer调用相关工具继续检索答案
                 2.如果能够很好的满足用户当前用户问题的需求，进入Answer给出最终答案
                 ... ( infer/Observation  至少可以循环迭代优化 1 次 ，最多可以迭代优化 4 次)
            3. Answer: 已经完成了前置问题的检索，应该给出一个结构完整，格式清晰，内容充实，易于阅读的最终答案
        ...
        返回值说明:
          ...
          - `reply`: 每个步骤对应的答案
              - infer: 字段描述 -> {{"tool_name":<string>,"args": <list>}}, 输出需要调用的工具 
                   - `tool_name` -> 工具名称
                   - `args` -> 工具所需的参数列表
              - Observation: 字段描述 -> {{"result":<string>,"next_step":<string>}}, 输出当前观察结果的优缺点，给出接下来执行的操作 infer/Answer
                   - `result`: 观察结果，应包含检索问题的优缺点及对解决用户问题的满意程度
                   - `next_step`: 如果需要继续优化则为 infer， 如果无需继续优化则为 Answer
    ...
    def query_desc(self) -> str:
        desc = """
         ReSearchAgent -> 这是一个基于深度搜索的问答Agent，用更长的时间和更多的检索来源 (网络检索和知识库检索) 来保证回答问题的准确性和全面性，可以用于对回答对严谨性，准确性，权威性，全面性有更高要求的问题，返回基于深度检索的问题答案
         """
        return desc
    ...
    async def action(self, chat_no: str, json_data: dict):
        ...
        if json_data["step"] == "infer":
             ...
             search_result = await self.query_search(chat_no, search_data)
             search_result = str(search_result)
             self.append_message(chat_no, {"role": "assistant", "content": f"""工具执行结果: {search_result} """})
             return await self.exec0(chat_no)
        elif json_data["step"] == "Observation":
            reply = ast.literal_eval(json_data["reply"])
            if reply["next_step"] == "infer":
                self.append_message(chat_no, {"role": "assistant", "content": f"""当前检索信息存在不足，请根据观察结果，重新进行推理优化 """})
            return await self.exec0(chat_no)
